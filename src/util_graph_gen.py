...
def specify_relation(first_word, relation, reader):
    if reader is None or reader.points is None:
        cnt_points_reader = 0
    else:
...
def specify_root_selector(reader):
    ...
    if root_selector != 'rs-arbitrary':
        root_selector = specify_relation(None, root_selector, reader)
